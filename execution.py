...
class PromptQueue:
    ...
    def get_queue_file(self):
        ...
        queue_dir = os.path.join(current_dir, 'tmp_queues')
    ...
    def save_queue(self):
        ...
        print("Saving queue")
        with self.mutex:
            # Move any running tasks back to queue
            if self.currently_running:
                for task_id, task in self.currently_running.items():
                    heapq.heappush(self.queue, task)
            
            # Remove duplicates based on prompt_id (UUID)
            seen_prompt_ids = set()
            unique_queue = []
            for item in self.queue:
                prompt_id = item[1]  # prompt_id is at index 1 in the tuple
                if prompt_id not in seen_prompt_ids:
                    seen_prompt_ids.add(prompt_id)
                    unique_queue.append(item)
            
            # Convert tuples to lists for JSON serialization
            queue_data = [list(item) for item in unique_queue]
            try:
                queue_file = self.get_queue_file()
        ...
        with self.mutex:
            try:
                ...
                with open(queue_file, 'r') as f:
                    ...
                    # Convert lists back to tuples and maintain heap property
                    self.queue = [tuple(item) for item in queue_data]
                    heapq.heapify(self.queue)  # Ensure queue maintains heap property
                    print("self.queue count: ", len(self.queue))
                return True
            except Exception as e:
    ...
    def move_running_to_queue(self):
        """Move all currently running tasks back to the queue"""
        with self.mutex:
            if not self.currently_running:
                logging.info("No running tasks to move")
                return False
            
            # Move all running tasks back to queue
            for task_id, task in self.currently_running.items():
                heapq.heappush(self.queue, task)
            ...
            logging.info(f"Moved {len(self.queue)} tasks back to queue")
            return True
    ...
    def start_task(self):
        ...
        with self.mutex:
            ...
            # Unpack the queue item (number, prompt_id, prompt, extra_data, outputs_to_execute)
            number, prompt_id, prompt, extra_data, outputs_to_execute = item
            
            # Add to currently running with task counter
            ...
            logging.info(f"Started task {prompt_id}. Queue length: {len(self.queue)}, Running tasks: {len(self.currently_running)}")
            return True
